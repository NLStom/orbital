"""
Vertex AI (Google Cloud) LLM provider.

Implements the LLMProvider interface for Gemini models via Vertex AI.
Uses API key authentication with Vertex AI mode.
"""
...
class VertexAIProvider(LLMProvider):
    """Vertex AI provider using Google Cloud API key."""
    ...
    def __init__(self, api_key: str, model_id: str):
        ...
        Args:
            api_key: Google Cloud API key (bound to service account)
            model_id: Model identifier (e.g., 'gemini-2.5-flash')
        """
        # Set env vars for the Google SDK to pick up
        # (SDK reads from os.environ, not from constructor args for Vertex AI mode)
    ...
    async def generate(
        ...
        logger.debug(f"[VertexAI] Calling model {self.model_id} with {len(contents)} contents")
        ...
        try:
            ...
            logger.debug(f"[VertexAI] Response received: {len(response.candidates)} candidates")
            return self._parse_response(response)
        except Exception as e:
            logger.error(f"[VertexAI] API error: {type(e).__name__}: {e}")
            raise
    ...
    def _parse_response(self, response: Any) -> LLMResponse:
        ...
        logger.info(f"[VertexAI] Raw response: {response}")
